'''
python main_segmentation.py --data_set DRIVE --proxy_dir ./BenchmarkTransferLearning-main/pretrained_models --init MAGICAL-10
...
import torch
from test import segmentation_engine
...
def get_args_parser():
    ...
    parser.add_argument('--train_num_workers', help='train num of parallel workers for data loader', default=8,
                        type=int)
    parser.add_argument('--test_num_workers', help='test num of parallel workers for data loader', default=8, type=int)
    ...
    parser.add_argument('--patience', type=int, default=30, help='num of patient epochesr')
...
def main(args):
    if args.init == 'moco-v3':
        args.proxy_dir = os.path.join(args.proxy_dir, args.init+'.tar')
    ...
    if args.init is not None and args.init.lower() != 'imagenet' and args.init.lower() != 'random':
    ...
    if args.data_set == "DRIVE":
        ...
        criterion = torch_dice_coef_loss#torch.nn.BCELoss()#
        segmentation_engine(args, model_path, dataset_train, dataset_val, dataset_test,criterion)
