...
def get_encoder(name, in_channels=3, depth=5, weights=None):
    ...
    logs_path = "./pretrained_models/Logs"
    log_writter = open(os.path.join(logs_path, "log.txt"), 'w')

    if weights is not None:
        print(weights)
        if not weights.lower().endswith("imagenet.pth"):
            if 'random' not in weights:
                checkpoint = torch.load(weights, map_location="cpu")
                if "state_dict" in checkpoint:
                    state_dict = checkpoint["state_dict"]
                else:
                    state_dict = checkpoint
                print('my test',state_dict.keys(), file=log_writter)
                state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
                state_dict = {k.replace("encoder_q.", ""): v for k, v in state_dict.items()}
                state_dict = {k.replace("encoder.", ""): v for k, v in state_dict.items()}  # new
                for k in list(state_dict.keys()):
                    if k.startswith('fc') or k.startswith('classifier') or k.startswith('projection') or k.startswith('prototypes') or k.startswith('encoder_k') or k.startswith("queue") or k.startswith('projector') :
                        del state_dict[k]
                        continue
                    '''
                    update for load the encoder of MG
                    '''
                    if k.endswith('num_batches_tracked') or k.startswith('decoder'):
                        del state_dict[k]
                ...
                # _msg = encoder.load_state_dict(state_dict)
                # if len(_msg.missing_keys) != 0:
                #     print('_msg.missing_keys is :',_msg.missing_keys)
                print('my test for differ',state_dict.keys(), file=log_writter)
                print("=> loaded pre-trained model from'{}'".format(weights))
