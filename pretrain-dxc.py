...
def main(conf):	
	...
	print(conf)
	conf.train_list = './datasets/CheXpert-v1.0-small/train_new.csv'
	conf.train_list_txt = './datasets/CheXpert-v1.0-small/train_new.txt'
	conf.val_list = './datasets/CheXpert-v1.0-small/valid_new.csv'
	conf.val_list_txt = './datasets/CheXpert-v1.0-small/valid_new.txt'

	dataset_train = CheXpertDataset_pretrain(images_path=conf.data, file_path=conf.train_list_txt,
										augment=build_transform_classification(normalize=conf.normalization, mode="train"), uncertain_label='LSR-Ones', unknown_label=0)

	dataset_valid = CheXpertDataset_pretrain(images_path=conf.data, file_path=conf.val_list_txt,
										augment=build_transform_classification(normalize=conf.normalization, mode="valid"), uncertain_label='LSR-Ones', unknown_label=0)
	...
	for epoch in range(intial_epoch,conf.nb_epoch):
		...
		for iteration, (image,imagePath) in pbar:
