...
def get_args_parser():
	...
	parser.add_argument('--data', default="./datasets/DRIVE")
...
def main(conf):	
	...
	print(conf)
	train_img_dir = os.path.join(conf.data,'train/images')
	valid_img_dir = os.path.join(conf.data,'test/images')
	train_mask_dir = os.path.join(conf.data,'train/labels')
	valid_mask_dir = os.path.join(conf.data,'test/labels')
	dataset_train = DriveDataset(train_img_dir,train_mask_dir)
	dataset_valid = DriveDataset(valid_img_dir,valid_mask_dir)
	...
	for epoch in range(intial_epoch,conf.nb_epoch):
		...
		for iteration, (image_tuple) in pbar:
			...
			image, mask = data_augmentation(image, mask, conf.flip_rate)
